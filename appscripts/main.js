...
commentary.innerHTML = `<hr>This webpage is powered by HTML5, CSS, and purely by the Javascript library, Raphaël.<br><br>All audio and SVG line paths are extracted from works that are in the public domain. The image files used are created by yours truly.<br><br> © 2021, Lenon Ong`;
    ...
    hitAudio[5].play();
			...
			theDoor.node.addEventListener("click", function myFunction(ev){
				...
				//commentary.innerHTML = `[${new Date().toLocaleTimeString()}] <b>The Talking Lemon</b>: This is the endgame. We are now in the <i>internal dimension</i> of a patent, and this is the manifestation of the patent's core. <i>Fancy.</i> You can learn more about the principles of "inventive step" and "industrial application" here. <br><br><br> Y'know, this is inspired by the internal dimension where Naruto talks to the Nine Tailed Fox:<br>Source:<a href='https://www.youtube.com/watch?v=vWwVkkzruFM 'target= '_blank'><span class='underline'>YouTube - "Naruto Meets Kyuubi First Time"</span></a>
					    		//<hr>This webpage is powered by HTML5, CSS, and purely by the Javascript library, Raphaël.<br><br>All audio and SVG line paths are extracted from works that are in the public domain. The image files used are created by yours truly.<br><br> © 2021, Lenon Ong`;
				hitAudio[4].pause();
...
let startGame04 = function(){
    ...
    theEgg.node.addEventListener("click", function myFunction(ev){
		...
		overlay2.animate({opacity: 0}, 10000, function(ev){this.remove();});
		mouseText2.remove();
		...
		mouseText3.animate({opacity: 0}, 10000, function(ev){this.remove();});
		var scoreTextStage4 = paper.text(pWidth/4*3.6, pHeight/10, `Black orbs cleared: ${orbsCleared} / 100`).attr({"font-size": 15, "fill": "#000000", "font-family": "Arial", "opacity": 0});
	...
	let createBubbles = function(){			
		...
		while (bubblesCounter < numBubbles) {
				...
				newBubbles[bubblesCounter].animate({"cx": map(Math.random(),0,1,-20,pWidth), "cy": -pHeight/5}, map(Math.random(),0,1,10000,20000), function(ev){this.remove();});
				bubblesCounter++;
		};
	};
		...
		let updateMap = function() {
			...
			scoreTextStage4.attr({text: `Black orbs cleared: ${orbsCleared} / 99`});
			...
			if(playerAvatar.posy >= pHeight-21){
			...
			if(playerAvatar.posx >= pWidth-21){
			...
			if(playerAvatar.posy <= -playerAvatarRadius){
				playerAvatar.speedY += 10;
				clashInitialRipple.attr({
	            	"cx": playerAvatar.posx,
	            	"cy": playerAvatar.posy
		...
		let createBubblesInverval = setInterval(createBubbles, 10000);
